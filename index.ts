 ...
 */
export function parse(policy: string): Map<string, Set<string>> {
    ...
    return new Map(policy.split(';')
        ...
        .filter(([name, _]) => unseenName(name)) as any
    )
}
 ...
 */
export function serialize(policy: Map<string, Set<string>>): string {
